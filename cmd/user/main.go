...
func main() {
	...
	if err := config.Validate(cfg); err != nil {
		log.Fatalf("user invalid configuration: %v", err)
	}
	log.Printf("user config: %s", config.FormatConfig(cfg))
	...
	if err != nil {
		log.Fatalf("user service failed to init telemetry: %v", err)
	}
	defer func() {
		shutdownCtx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
		defer cancel()
		if err := shutdownOTel(shutdownCtx); err != nil {
			log.Printf("user telemetry shutdown error: %v", err)
		}
	}()
