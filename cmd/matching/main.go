...
func main() {
	...
	if err := config.Validate(cfg); err != nil {
		log.Fatalf("matching invalid configuration: %v", err)
	}
	log.Printf("matching config: %s", config.FormatConfig(cfg))
	...
	if err != nil {
		log.Fatalf("matching readiness init failed: %v", err)
	}
	defer func() {
		shutdownCtx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
		defer cancel()
		for _, closer := range ready.Closers {
			...
			if err := closer(shutdownCtx); err != nil {
				log.Printf("matching readiness close error: %v", err)
			}
		}
	}()
